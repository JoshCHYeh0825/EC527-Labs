/*****************************************************************************/
// gcc -O1 test_mmm.c -lrt -o test_mmm
...
#define CPNS 3    /* Cycles per nanosecond -- Adjust to your computer,
...
void bmm_ijk(matrix_ptr a, matrix_ptr b, matrix_ptr c, int bsize);
void bmm_kij(matrix_ptr a, matrix_ptr b, matrix_ptr c, int bsize);
void bmm_jki(matrix_ptr a, matrix_ptr b, matrix_ptr c, int bsize);
...
int main(int argc, char *argv[])
{
  ...
  printf("Dense BMM tests \n\n");

  wakeup_answer = wakeup_delay();

  printf("Doing BMM three different ways,\n");
  ...
  // Parameterized block sizes
  int block_sizes[] = {16, 64};
  ...
  for (int b = 0; b < num_blocks; b++) {
    ...
    if (OPTIONS > 2) {
      for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
        ...
        set_matrix_row_length(c0, n);
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
        bmm_jki(a0, b0, c0, bsize);
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
        time_stamp[OPTION][x] = interval(time_start, time_stop);
      }
    }

    printf("Done collecting measurements for block size %d:\n\n", bsize);
