/***********************************************************************
 ...
 gcc -O1 -fopenmp mmm_omp.c -lrt -o mmm_omp
 OMP_NUM_THREADS=4 ./mmm_omp
...
int main(int argc, char *argv[])
{
  ...
  for (OPTION = 0; OPTION<OPTIONS; OPTION++) {
    ...
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
      ...
      switch(OPTION) {
        case 0:           mmm_kij_omp(a0, b0, c0);     break;
        default: break;
      }
...
#pragma omp parallel shared(a0,b0,c0,row_length, i,j,k,r)
  {
  #pragma omp for
