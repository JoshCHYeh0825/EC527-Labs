...
#define A   120  /* coefficient of x^2 */
#define B   28  /* coefficient of x */
...
void lmnt_add(data_t* pA1, data_t* pA2, data_t* PR, long int nSize);
void lmnt_mult(data_t* pA1, data_t* pA2, data_t* PR, long int nSize);
void AVX_dot(data_t* pA1, data_t* pA2, data_t* PR, long int nSize);
...
int main(int argc, char *argv[])
{
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    for (long k=0; k<OUTER_LOOPS; k++) {
      lmnt_add(pArray1, pArray2, pResult, n);
    }
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  }
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    for (long k=0; k<OUTER_LOOPS; k++) {
      lmnt_mult(pArray1, pArray2, pResult, n);
    }
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  }
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    for (long k=0; k<OUTER_LOOPS; k++) {
      AVX_dot(pArray1, pArray2, pResult, n);
    }
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  }
  ...
  /* output times */
  printf("size, Scalar, vecSSE, vecAVX, lmnt_add, lmnt_mult, AVX_dot\n");
  {
    for (int i = 0; i < x; i++) {
...
/* non-vector element-wise addition*/
void lmnt_add(data_t* pArray1,  // [in] 1st source array
  data_t* pArray2,              // [in] 2nd source array
  data_t* pResult,              // [out] result array
  long int nSize)               // [in] size of all arrays
{
...
/* non-vector element-wise multiply*/
void lmnt_mult(data_t* pArray1,  // [in] 1st source array
  data_t* pArray2,              // [in] 2nd source array
  data_t* pResult,              // [out] result array
  long int nSize)               // [in] size of all arrays
{
...
/*vector dot product with intrinsics*/
void AVX_dot(data_t* pArray1, data_t* pArray2, data_t* pResult, long int nSize){
  
  long int i;
  long nLoop = nSize / 4;
  __m128 sum_vec = _mm_setzero_ps();  // Initialize vector accumulator

  for (i = 0; i < nSize - 4; i += 4){
    __m128 v1 = _mm_loadu_ps(&pArray1[i]);
    __m128 v2 = _mm_loadu_ps(&pArray2[i]);

    __m128 dp = _mm_dp_ps(v1, v2, 0xF1);  // Compute dot product (sums lower 4 elements)
    sum_vec = _mm_add_ps(sum_vec, dp);  // Accumulate into sum_vec
  }

  // Final reduction from SIMD register to scalar
  sum_vec = _mm_hadd_ps(sum_vec, sum_vec); // Sum adjacent pairs
  sum_vec = _mm_hadd_ps(sum_vec, sum_vec); // Sum last two elements

  // Store the final result
  _mm_store_ss(pResult, sum_vec);
}
